...
def read_additional_features(graphA, graphB, features_dir):
    source_features = align_feature_dict(
        graphA,
        load_feature_dict(
            "/home/fabian/Desktop/MP/repos/schematch/" + features_dir + "/source.json"
        ),
    )
    target_features = align_feature_dict(
        graphB,
        load_feature_dict(
...
def get_embeddings(graphA, source_graph_file, graphB, target_graph_file, features_dir):
    if (source_graph_file, target_graph_file) in EMBEDDINGS_CACHE:
        embeddings_source, embeddings_target = EMBEDDINGS_CACHE[
            (source_graph_file, target_graph_file)
        ]
    else:
        if EMBEDDING_GENERATION == "N2V":
            ...
            addFeaturesA, addFeaturesB = read_additional_features(
                graphA, graphB, features_dir
            )
            ...
            embeddings_combined = embed_xnetmf(
                combined_graph, np.concatenate((featuresA, featuresB), axis=0)
            )
...
def match(
    source_graph_file, source_table, target_graph_file, target_table, features_dir
):
    ...
    embeddings_source, embeddings_target = get_embeddings(
        graphA, source_graph_file, graphB, target_graph_file, features_dir
    )
